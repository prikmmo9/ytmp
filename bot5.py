# bot.py - Основной файл бота с мониторингом каналов
import os
import asyncio
import logging
import threading
...
MONITOR_INTERVAL_MINUTES = 30  # Как часто проверять каналы
ENABLE_MONITORING = True  # Включить мониторинг при запуске
...
async def check_all_channels(send_notifications: bool = True) -> list:
    ...
    for channel in channels[:20]:  # Максимум 20 каналов за раз
...
async def channels_handler(event):
    ...
    if not channels:
        await event.reply("📭 **Нет каналов в базе данных**\n\nКаналы добавляются автоматически при скачивании видео.")
        return
...
async def callback_handler(event):
    ...
    if cached and cached.get('storage_message_id') and cached.get('storage_chat_id'):
        try:
            ...
            logger.info(f"⚡ Переслано из хранилища: {cached['title'][:50]}...")
            return
        except Exception as e:
            logger.warning(f"⚠️ Ошибка пересылки из кэша: {e}. Качаем заново...")
                    ...
                    storage_message = await client.send_file(
                        ...
                        attributes=[
                            telethon.types.DocumentAttributeAudio(
                                ...
                                title=video_info.get('fulltitle', '')[:100],
...
async def main():
    ...
    try:
        ...
        logger.info(f"🗄 Хранилище: @copirkaDva (id={storage_chat_id}) ✅")
    except Exception as e:
        logger.error(f"❌ Хранилище недоступно: {e}")
        logger.warning("⚠️ Бот будет работать без хранилища")
    ...
    print(f"  /stats | /monitor | /channels | /database | /cancel")
    print("=" * 60)
    print()
